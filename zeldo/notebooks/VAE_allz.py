...
if input_dim == 128: 
    ...
    for i in snapshot_range: 
        ...
        read_data = np.log10(read_data + 100)# + 0.0001   ### setting an offset so that log10 doesn't give -inf.

        
        if i == snapshot_range[0] :
            temp_data=read_data[0:num_sim_per_snap -1 ,:,:]
            temp_idx = i*np.ones(shape=read_data.shape[0])
        
        temp_data = np.concatenate((temp_data, read_data[0:num_sim_per_snap - 1,:,:]))   #each timestep we take 1000, and append all together.
        #print(read_data[0:999,:,:])
        temp_idx = np.concatenate((temp_idx, i*np.ones(shape=read_data.shape[0]) ))
