...
swe_train = preproc.fit_transform(swe_data[:9000,:])
swe_valid = preproc.transform(swe_data[9000:,:])
...
for indx in range(8):
    ...
    plt.savefig(folder+'VAE_gen'+str(indx)+'.png')
    plt.clf()
    np.save(folder+'VAE_gen_'+str(indx), generator[indx,:,:,0])
